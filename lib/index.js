...
function syncthing(options) {
  ...
  if (config.eventListener) {
    ...
    //Start listening
    _eventCaller2.default.call(instance, req, config.retries, config.events);
  }
...
var methods = function methods(req) {
  return {
    ...
    db: {
      ...
      revert: function revert(folder, cb) {
        var attr = [{ key: 'folder', val: folder }];
        return req({ method: 'db', endpoint: 'revert', post: true, attr: attr }, cb);
      }
    },
    stats: {
    ...
    misc: {
      ...
      randomString: function randomString() {
        ...
        var attr = [{ key: 'length', val: length }];
        return req({ method: 'svc', endpoint: 'random/string', attr: attr }, cb);
      }
    }
  };
};
